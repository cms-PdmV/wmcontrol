...
# GF
		elif sequence == 'run,fromFileSource':
			if hasattr(self.process.source,'firstRun'):
				#self.executeAndRemember('process.load("HLTrigger.Configuration.hlt_run%d_cff"% process.source.setRunNumber.value() )')
				#self.executeAndRemember('process.load("HLTrigger.Configuration.hlt_run%%d_cff"%%process.source.firstRun.value() )' )
				self.executeAndRemember('process.load("HLTrigger.Configuration.hlt_run%d_cff"% process.source.firstRun.value() )')
			elif hasattr(self.process.source,'setRunNumber'):
				#self.executeAndRemember('process.load("HLTrigger.Configuration.hlt_run%%d_cff"%%(process.source.setRunNumber.value()) )' )
				#self.executeAndRemember('process.load("Configuration.Menu.hlt_run%d_cff"% process.source.setRunNumber.value() )')
				self.executeAndRemember('process.load("HLTrigger.Configuration.hlt_run%d_cff"% process.source.setRunNumber.value() )')
# GF
		else:
