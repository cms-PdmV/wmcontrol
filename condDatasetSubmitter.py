...
def isAtSite(ds, run):
  ...
  blockDicts = json.loads(wma.httpget(connection, wma.DBS3_URL + "blocks?dataset=%s&run_num=%s" % (
      ds,run))) # connection return a string which represents a list
  ...
  for c in confCondList:
    ...
    if 'recodqm' in details:
      ...
      if options.recoCmsswDir:
        cmssw_command = "cd %s; eval `scramv1 runtime -sh`; cd -" % options.recoCmsswDir
        upload_command = "wmupload.py -u %s -g PPD -l %s %s" % (os.getenv('USER'),
            'step4_HARVESTING.py','step4_HARVESTING.py')
