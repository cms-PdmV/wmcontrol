...
def patch_configuration_file(cfg_name):
    """
    Patch a given configuration file.

    Args:
        cfg_name (str): Autogenerated cms-sw configuration fragment file.

    Returns:
        str | None: Path to the patched configuration file, `None` if
            the size of the configuration file is lower than a threshold,
            thefore it is not required to patch it, or if it is not possible
            to create a patched file.
    """
    size_limit = 1024 * 1024 * 7 # 7 MiB
    if os.path.getsize(cfg_name) < size_limit:
        return None
    ...
    patcher = FilterFilesFromConfigFile()
    return patcher.patch(cfg_name)
