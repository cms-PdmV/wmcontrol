    ...
    if True:
        try:
            ...
            if e.errno != errno.ENOENT:
                logging.error('Impossible to open file %s (for other reason than not existing)',
                        metadataFilename)
...
I will ask you some questions to fill the metadata file. For some of the questions there are defaults between square brackets (i.e. []), leave empty (i.e. hit Enter) to use them.'''
    ...
    with open(metadataFilename, 'rb') as metadataFile:
        ...
        try:
            if metadata['HLT_release']:
                commands.append('./wmcontrol.py --req_file HLTConditionValidation_%s_%s_%s.conf' % (
                        metadata['HLT_release'], metadata['options']['basegt'],
                        metadata['options']['run']))

        except KeyError:
            commands.append('./wmcontrol.py --req_file PRConditionValidation_%s_%s_%s.conf' % (
                    metadata['PR_release'], metadata['options']['newgt'],
                    metadata['options']['run']))
